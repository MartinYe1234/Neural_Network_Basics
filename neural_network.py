...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        :param eta: float
            float representing the learning rate of the network, the bigger it is the bigger the adjustments the network
            will make
        :return:
        """
        ...
        for x, y in mini_batch:
            delta_nabla_b, delta_nabla_w = self.backprop(x, y)  # determine what changes need to be made to the gradients
            nabla_b = [nb + dnb for nb, dnb in zip(nabla_b, delta_nabla_b)]  # apply the changes
            nabla_w = [nw + dnw for nw, dnw in zip(nabla_w, delta_nabla_w)]  # apply the changes
    ...
    def backprop(self, x, y):
        ...
        for l in range(2, self.num_layers):
            z = zs[-l]  # start from second last layer and go backwards all the way to the first layer
            sp = sigmoid_prime(z)
...
def train_network():
    ...
    net.SGD(training_data, 35, 5, 0.025, test_data=test_data)

# uncomment in order to train a new network
# train_network()
